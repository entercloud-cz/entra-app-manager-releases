...
- **This page describes 3.8.1.** It is generated from that release's own guide — an older release's
  page is the `INSTALL.md` attached to it.
- **Every release:** <https://github.com/entercloud-cz/entra-app-manager-releases/releases>
- **The image:** `ghcr.io/entercloud-cz/entra-app-manager:v3.8.1`
...
| The address people will open | only if it is **not** the hostname Azure generates. It is asked once and used twice: Entra refuses a redirect URI it was never told about, and the deployment itself has to be told, because anything you put in front of it (a Front Door, a gateway) hides the browser's address from the application. The generated hostname is always registered, and nothing means "that one" |
...
**A public reference, which is the ordinary case and needs no credential at all:**
    ...
    ghcr.io/entercloud-cz/entra-app-manager:v3.8.1
...
# 2. the subscription owner — the deployment itself. -PublicFqdn is the address people will open; leave it out if
#    that is the hostname Azure generated. It belongs here because the container is built from it
.\install.ps1 -Only preflight,deploy -SubscriptionId <id> -Location swedencentral -PublicFqdn eam.contoso.com

# 3. the Entra administrator again — the redirect URIs and the federated credential need the deployed hostname and
#    the deployed identity, which is why this is not part of step 1. The address from step 2 is remembered, so it
...
**And if people will open a different address, say so when the installer asks.** It asks in the `deploy` phase, and
the answer does two things. Entra matches a redirect URI exactly and refuses one it was never told about — naming
the URI, not the omission — so a deployment behind a custom domain cannot be signed in to until that address is
registered. Both are registered, sign-in and sign-out paths for each, and the generated hostname is never replaced.
Entra's **front-channel logout URL** is a single value and points at the address you gave; that only matters when
something else signs a person out, and sign-out from the product works on both.

**The deployment is also told the address, and that is why the question comes before the deploy.** Anything you put
in front of the ingress that rewrites the address on its way in — Azure Front Door, an Application Gateway, a reverse
proxy of your own — passes the application *its* address rather than the one in the browser's bar, because that is
what such a thing does. So the application is given the address instead of guessing it from the request, which it
deliberately will not do: on an ingress reachable from the internet, believing a header about who you are asking for
is how somebody else decides where a sign-in ends up.

**You need this only if something rewrites the address.** A deployment people open at the hostname Azure generated,
and one whose custom domain is bound to the container app itself, both already see the right address; nothing is set,
nothing changes, and there is nothing to do. If you do need it and the deployment has not been told, sign-in returns
people to the generated hostname and can fail with *Correlation failed* — the fix is `-Only deploy` with the address,
or the same variable set by hand:

```powershell
az containerapp update --name ca-eam-web-prod --resource-group rg-eam-prod `
  --set-env-vars SignIn__PublicHost=eam.contoso.com
```

Set by hand it survives an upgrade, which does not touch the deployment. A later `-Only deploy` re-asserts whatever
the installer was told, so keep the two in step — the installer remembers your answer, and clearing it clears the
variable.

Registering an address does not create it: the custom domain still has to resolve to the deployment.
...
is worse than plain text. *Yes, the installer asked for an address too, and no, this one does not replace it: that
one decides where a sign-in returns to and has to be known before anybody can sign in at all, which is why it lives
in the deployment and not on this screen. Normally you type the same address in both.*
...
- **What it cannot do is change the deployment itself.** An upgrade rolls an image; it does not deploy a template, so
  it cannot add or change a setting the deployment holds. A release whose notes are marked *action required* is
  telling you there is a step this command does not contain; one that is not so marked can be taken as it comes, and
  anything new it offers that needs a setting is offered rather than required.
